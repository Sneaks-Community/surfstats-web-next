...
**Caching.** Valkey (Redis-compatible), cache-aside. Client in [`lib/valkey.ts`](lib/valkey.ts) and [`lib/valkey-cache.ts`](lib/valkey-cache.ts) — the only two `valkey-*` files, since everything above them is per-domain. A domain's queries and its cache wrappers live in one `lib/*-cache.ts` (map, registry, dashboard, country, player, map-records, map-stats); [`lib/server-status.ts`](lib/server-status.ts) is the same thing for the GameDig-backed live status. All keys use the `surfstats:` prefix. Keys or TTLs that more than one module has to agree on live in [`lib/cache-keys.ts`](lib/cache-keys.ts); build them from there, never as a second literal. If Valkey is unreachable, `proxy.ts` serves a "temporarily unavailable" page rather than running uncached queries.
