...
* `RATE_LIMIT_PREFETCH_MAX`: Max router link prefetches per window per IP (default: `900`). Own budget, since one page view prefetches every viewport `<Link>` (~39 on the home page). Note this is the effective per-IP ceiling on full page renders, not `RATE_LIMIT_PAGE_MAX`: the scope is chosen from `Sec-Fetch-Dest`, which a client can send freely, and a request claiming `empty` still gets a full document. Lowering it below what real browsing needs is the only way to lower that ceiling.
* `RATE_LIMIT_WINDOW_SECONDS`: Window length in seconds (default: `60`).
* `RATE_LIMIT_BLOCK_SECONDS`: How long an over-budget IP stays blocked, timed from the moment it blows the budget rather than from the end of its window, and replacing that window's reset (default: `0`, disabled). `Retry-After` reports the block, so a value larger than `RATE_LIMIT_WINDOW_SECONDS` means clients are told to wait longer than the window. Use it to make sustained abuse expensive without lowering the budget for everyone.
...
* `DB_STATEMENT_TIMEOUT_MS`: Server-side cap per statement, applied to every pooled connection (`max_statement_time` on MariaDB, `max_execution_time` on MySQL). A query that exceeds it is killed and its connection released, instead of running on after the client has given up; `0` disables (default: `8000`). It also bounds the [expensive-query queue](lib/db-semaphore.ts): a waiter sits behind at most two of these, so a high value turns a burst into a long tail. Raise it only if legitimate cold queries are being killed.
